...
def hid_graph(ax_hid, dict_linevis,
    ...
    for i in range(len(additional_HLD_points_flux)):
        ...
        line_match_mask=np.array(additional_line_points[additional_line_points.columns[:4]] == point_row[:4]).all(1)

        line_match_row=line_match_mask.all()

        point_row_det=False
        point_no_line_info = 0
        ...
        if line_match_row:
            ...
            point_row_size=np.max(additional_line_points[[True,False]][['EW_FeKa25','EW_FeKa26']])
            point_row_det=point_row_size!=0.
            if not point_row_det:
                point_row_UL_size=np.max(additional_line_points[[True,False]][['EW_FeKa25_UL','EW_FeKa26_UL']])
        ...
        if np.sum([point_row[column] in [0., None] for column in column_3_10_df]) == 0:
            ...
            if not broad_mode:
                ax_hid.scatter(point_LEdd_HR_soft, point_LEdd_3_10,
                               ...
                               color=None if not point_no_line_info and not point_row_det
                               else (point_row['color'] if point_row['color'] not in ['', None] else 'black'),
                               # abs marker for points with line infos
                               marker="P" if point_no_line_info else marker_abs,
                               s=15 if point_no_line_info else
                               norm_s_lin * point_size_EW ** norm_s_pow,
                               edgecolor=(point_row['color'] if point_row['color'] not in ['', None] else 'black')
                               if not point_no_line_info and not point_row_det else 'black', zorder=1e6)
